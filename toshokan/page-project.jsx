...
/* global React, Icon, Node, Slot, SpecimenFrame, TechChip, PathChip, StatusChip, PrimaryBtn, SecondaryBtn, IconBtn, TSK_TREE, TSK_FILE_PREVIEWS */

const { useState, useMemo } = React;

// ---- Overview column --------------------------------------------------------
function IdentityCard({ project, activeServiceCount, onLaunch, onAction }) {
  const actions = [
  ...
  return (
    <section style={{
      ...
      borderRadius: "var(--r-panel)", borderTopRightRadius: 5,
      boxShadow: "var(--shadow-tile)", padding: 22, position: "relative"
    }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 16 }}>
        ...
        }}><Icon name="branch" size={12} /> {project.branch}</span>
        {activeServiceCount > 0 && (
          <span style={{
            display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 9px",
            background: "var(--surface)", border: "1px solid var(--line-soft)", borderRadius: "var(--r-chip)",
            fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--text-secondary)"
          }}><Icon name="pulse" size={12} /> {activeServiceCount} active</span>
        )}
      </div>
...
function ActionRow({ label, icon, last, onClick }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{
        ...
        transition: "background 130ms ease", textAlign: "left"
      }}>
      {label}
...
function SummaryPanel({ project, onAction }) {
  const s = project.summary;
  return (
    <section style={{
      background: "var(--surface)", border: "1px solid var(--line-soft)",
      borderRadius: "var(--r-panel)", boxShadow: "var(--shadow-tile)",
      padding: "18px 20px", position: "relative", overflow: "hidden"
    }}>
      {/* exposed seam along top edge — documentation as attached layer */}
...
function SubprojectsPanel({ project, onLaunch }) {
  if (!project.subprojects.length) return null;
  return (
    <section style={{
      background: "var(--surface)", border: "1px solid var(--line-soft)",
      borderRadius: "var(--r-panel)", boxShadow: "var(--shadow-tile)", padding: "16px 20px"
    }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
...
function TreeRow({ node, depth, expanded, selected, onToggle, onSelect }) {
  ...
  return (
    <div role="row"
      ...
      style={{
        display: "grid", gridTemplateColumns: "1fr 86px 150px",
...
function DirectoryPanel({ project, onSelectFile, selectedFile }) {
  const [expandedPaths, setExpandedPaths] = useState({});
  const [dirQuery, setDirQuery] = useState("");

  const toggle = (path) => setExpandedPaths((prev) => ({ ...prev, [path]: !prev[path] }));
  ...
  return (
    <section style={{
      background: "var(--surface)", border: "1px solid var(--line-soft)",
      borderRadius: "var(--r-panel)", boxShadow: "var(--shadow-tile)", overflow: "hidden",
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderBottom: "1px solid var(--line-soft)" }}>
        ...
        <span style={{ marginLeft: "auto", display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{
            display: "flex", alignItems: "center", gap: 7, background: "var(--bg-base)",
            border: "1px solid var(--line-soft)", borderRadius: 8, padding: "5px 10px"
          }}>
            <Icon name="search" size={13} style={{ color: "var(--text-muted)" }} />
            <input value={dirQuery} onChange={(e) => setDirQuery(e.target.value)} placeholder="Search in directory..."
              style={{ border: "none", background: "transparent", fontSize: 12.5, width: 150 }} />
          </span>
      ...
      {/* column headings */}
      <div style={{
        display: "grid", gridTemplateColumns: "1fr 86px 150px", gap: 10,
        padding: "8px 16px 8px 39px", borderBottom: "1px solid var(--line-soft)",
      ...
      <div role="tree" aria-label="Project directory" style={{ overflowY: "auto", maxHeight: 430 }}>
        {rows.map(({ node, depth, expanded }) => (
          <TreeRow key={node._path} node={node} depth={depth} expanded={expanded}
            selected={selectedFile === node.name && node.type === "file"}
...
function FilePreview({ fileName, onAction }) {
  const preview = TSK_FILE_PREVIEWS[fileName];
  return (
    <section style={{
      background: "var(--surface)", border: "1px solid var(--line-soft)",
      borderRadius: "var(--r-panel)", boxShadow: "var(--shadow-tile)", overflow: "hidden"
      ...
      {preview ? (
        <div>
          <pre className="mono" style={{
            margin: 0, padding: "14px 18px", fontSize: 12.5, lineHeight: 1.7,
            color: "var(--text-primary)", whiteSpace: "pre-wrap", overflowX: "auto"
          }}>{preview.lines.join("\n")}</pre>
...
function TrayButton({ project, current, onSelect }) {
  const [hover, setHover] = useState(false);
  return (
    <button title={project.name} aria-label={`View ${project.name}`}
...
function SpecimenTray({ projects, currentId, onSelect }) {
  return (
    <nav aria-label="Switch project" style={{
      ...
      {projects.map((p) => (
        <TrayButton key={p.id} project={p} current={p.id === currentId} onSelect={onSelect} />
      ))}
    </nav>
  );
}
...
// ---- Page --------------------------------------------------------------------------
function ProjectPage({ project, projects, services, onSelect, onLaunch, onAction, onBrowse }) {
