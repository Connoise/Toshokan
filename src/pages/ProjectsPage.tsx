...
import { Node, SpecimenFrame, TechChip, PathChip, PrimaryBtn, SecondaryBtn, IconBtn } from "../components/shared";
import type { Project, Service, ServiceState } from "../ipc";

type RunningState = ServiceState | null;
...
// ---- Specimen tile ----------------------------------------------------------
function ProjectTile({ project, selected, runningState, onSelect, onOpen, onLaunch }: TileProps) {
  ...
  return (
    <div
      ...
      style={{
        ...
        borderRadius: "var(--r-tile)", borderTopRightRadius: 4, padding: "18px 18px 0",
        boxShadow: hover || selected ? "var(--shadow-raised)" : "var(--shadow-tile)",
        transform: hover ? "translateY(-2px)" : "none",
        transition: "box-shadow 150ms ease, transform 150ms ease, border-color 150ms ease",
        cursor: "pointer", display: "flex", flexDirection: "column",
        outline: selected ? "1px solid var(--signal-soft)" : "none", outlineOffset: 3,
      }}
    >
      ...
      {project.service && (
        <div style={{
          marginTop: 10, display: "flex", alignItems: "center", gap: 8, padding: "6px 9px",
          background: "var(--running-soft)", border: "1px solid var(--line-soft)", borderRadius: "var(--r-chip)",
        }}>
      ...
      {project.subprojects.length > 0 && (
        <div style={{ marginTop: 12, background: "var(--surface-recessed)", border: "1px solid var(--line-soft)", borderRadius: 9, padding: "8px 10px" }}>
          ...
          {project.subprojects.map((sp) => (
            <div key={sp.name} style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <IconBtn icon="play" title={`Launch ${sp.name}`} size={26} onClick={(e: MouseEvent) => { e.stopPropagation(); onLaunch(sp.name); }} />
            </div>
          ))}
        </div>
      )}
      ...
      <div style={{
        marginTop: "auto", display: "flex", alignItems: "center", gap: 8, borderTop: "1px solid var(--line-soft)",
        ...
        <span style={{ marginLeft: "auto", display: "flex", gap: 6 }}>
          <IconBtn icon="play" title={`Launch ${project.name}`} size={28} onClick={(e: MouseEvent) => { e.stopPropagation(); onLaunch(project.name); }} />
          <IconBtn icon="dots" title="More actions" size={28} onClick={(e: MouseEvent) => e.stopPropagation()} />
        </span>
      </div>
    </div>
  );
...
// ---- Recent / selected band -------------------------------------------------
function RecentBand({ project, badge, services, onLaunch, onOpen }: { project: Project; badge: string; services: Service[]; onLaunch: (n: string) => void; onOpen: (id: string) => void }) {
  ...
  return (
    <section aria-label="Selected project" style={{
      ...
      borderRadius: "var(--r-panel)", boxShadow: "var(--shadow-tile)", padding: "20px 24px",
      display: "grid", gridTemplateColumns: "minmax(260px, 1fr) minmax(180px, 320px) auto auto",
      gap: 24, alignItems: "center", overflow: "hidden",
    }}>
      <div style={{ position: "relative", zIndex: 1 }}>
        <span className="micro" style={{ display: "inline-block", padding: "2px 8px", borderRadius: 5, border: "1px solid var(--line-soft)", background: "var(--bg-base)", marginBottom: 10 }}>{badge}</span>
      ...
      <div style={{ alignSelf: "stretch", display: "flex", flexDirection: "column", gap: 6, justifyContent: "center", borderLeft: "1px solid var(--line-soft)", paddingLeft: 20, minWidth: 0 }}>
        <div className="micro" style={{ marginBottom: 2 }}>
          {active.length > 0 ? `${active.length} service${active.length > 1 ? "s" : ""} active` : "Services"}
        </div>
        {svcs.length === 0 && <span style={{ fontSize: 12.5, color: "var(--text-muted)" }}>No services configured</span>}
        ...
        {svcs.length > 3 && <span style={{ fontSize: 11.5, color: "var(--text-muted)" }}>+{svcs.length - 3} more</span>}
      </div>
...
// ---- Compact list row -------------------------------------------------------
function ProjectRow({ project, selected, runningState, onSelect, onOpen, onLaunch }: TileProps) {
  const [hover, setHover] = useState(false);
  return (
    <div role="button" tabIndex={0}
      ...
      style={{
        display: "grid", gridTemplateColumns: "44px minmax(140px, 1.1fr) 1.4fr minmax(180px, 1.2fr) 130px 80px",
        gap: 14, alignItems: "center", padding: "9px 16px",
        background: selected ? "var(--signal-soft)" : hover ? "var(--surface)" : "transparent",
        borderBottom: "1px solid var(--line-soft)",
        borderLeft: selected ? "2.5px solid var(--signal)" : "2.5px solid transparent", cursor: "pointer",
      }}>
      ...
      <span style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
        <IconBtn icon="play" title={`Launch ${project.name}`} size={26} onClick={(e: MouseEvent) => { e.stopPropagation(); onLaunch(project.name); }} />
        <IconBtn icon="dots" title="More actions" size={26} onClick={(e: MouseEvent) => e.stopPropagation()} />
      </span>
    </div>
  );
}
...
export function ProjectsPage({ projects, services, selectedId, query, onSelect, onOpen, onLaunch }: ProjectsPageProps) {
  const [sort, setSort] = useState("lastOpened");
  const [view, setView] = useState("grid");

  const stateFor = (pid: string): RunningState => {
  ...
  const recentMost = projects.find((p) => p.id === "idolmancer") ?? projects[0];
  const featured = projects.find((p) => p.id === selectedId) || recentMost;

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      {!query.trim() && featured && <RecentBand project={featured} badge={featured === recentMost ? "Recent" : "Selected"} services={services} onLaunch={onLaunch} onOpen={onOpen} />}
      ...
      {view === "grid" ? (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(248px, 1fr))", gap: 18, alignItems: "stretch", paddingBottom: 8 }}>
          {filtered.map((p) => (
            <ProjectTile key={p.id} project={p} selected={selectedId === p.id} runningState={stateFor(p.id)} onSelect={onSelect} onOpen={onOpen} onLaunch={onLaunch} />
          ))}
        </div>
      ) : (
        <div style={{ background: "var(--surface)", border: "1px solid var(--line-soft)", borderRadius: "var(--r-panel)", boxShadow: "var(--shadow-tile)", overflow: "hidden", marginBottom: 8 }}>
          {filtered.map((p) => (
            <ProjectRow key={p.id} project={p} selected={selectedId === p.id} runningState={stateFor(p.id)} onSelect={onSelect} onOpen={onOpen} onLaunch={onLaunch} />
          ))}
        </div>
      )}
    </div>
