...
function RamCell({ ram, live }: { ram: number | null; live: boolean }) {
  ...
  return (
    <span style={{ display: "flex", flexDirection: "column", gap: 4, minWidth: 0 }}>
      <span className="mono" style={{ fontSize: 12, color: live ? "var(--text-primary)" : "var(--text-muted)" }}>{ram} MB</span>
...
function PortChip({ port, live }: { port: string | null; live: boolean }) {
  if (!port) return <span style={{ color: "var(--text-muted)" }}>—</span>;
  return (
    <span className="mono" style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "3px 9px", background: "var(--bg-base)", border: "1px solid var(--line-soft)", borderRadius: 6, fontSize: 11.5, color: live ? "var(--text-primary)" : "var(--text-muted)" }}>
...
function LogConsole({ lines }: { lines: LogEntry[] }) {
  const colors: Record<string, string> = { g: "#69C184", b: "#7CB3E8", y: "#FFB45A", r: "#D5685B", "": "#D7DDDE" };
  return (
    <div style={{ background: "#14181B", borderRadius: 10, padding: "13px 16px", fontFamily: "var(--font-mono)", fontSize: 12, lineHeight: 1.85, overflowX: "auto", border: "1px solid rgba(0,0,0,0.4)", boxShadow: "inset 0 2px 8px rgba(0,0,0,0.35)", minHeight: 100 }}>
...
function DetailField({ label, value, mono = false, chip = false, icon = null }: { label: string; value: string; mono?: boolean; chip?: boolean; icon?: string | null }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "128px 1fr", alignItems: "center", gap: 10 }}>
      ...
      {chip ? (
        ...
        <span className="mono" style={{ fontSize: 11.5, background: "var(--bg-base)", border: "1px solid var(--line-soft)", borderRadius: 6, padding: "5px 9px", color: "var(--text-primary)", display: "inline-flex", alignItems: "center", gap: 8, justifyContent: "space-between", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {value}
...
function ServiceRow({ svc, expanded, onToggle, onStart, onStop, onRestart, onAction }: RowProps) {
  ...
  const isJob = svc.kind === "job";

  // job-aware uptime cell (PLAN Q7): jobs show next/last-run instead of uptime
  const uptimeText = isJob
    ? (svc.nextRun ? `next ${svc.nextRun}` : svc.lastRun ? `ran ${svc.lastRun}` : "scheduled")
  ...
  return (
    <div style={{ borderBottom: "1px solid var(--line-soft)" }}>
      <div role="button" tabIndex={0}
        ...
        style={{
          display: "grid", gridTemplateColumns: SVC_GRID, gap: 12, alignItems: "center", padding: "13px 16px",
          ...
          cursor: "pointer", transition: "background 130ms ease", position: "relative", overflow: "hidden",
        }}>
        {svc.status === "starting" && (
          <span aria-hidden="true" style={{ position: "absolute", top: 0, left: 0, right: 0, height: 2, background: "linear-gradient(90deg, transparent, var(--signal), transparent)", backgroundSize: "200% 100%", animation: "tsk-starting-sweep 1.8s linear infinite" }}></span>
        )}
        ...
        <span style={{ fontSize: 12.5, color: "var(--text-secondary)" }}>{uptimeText}</span>
      ...
      {svc.status === "failed" && svc.error && (
        <div style={{ margin: "0 14px 12px", padding: "9px 14px", background: "var(--error-soft)", border: "1px solid rgba(168, 83, 72, 0.25)", borderRadius: 9, display: "flex", alignItems: "center", gap: 12 }}>
...
export function ServicesPage({ services, query, onStart, onStop, onRestart, onAction, onBrowse }: ServicesPageProps) {
  ...
  return (
    <section style={{ background: "var(--surface)", border: "1px solid var(--line-soft)", borderRadius: "var(--r-panel)", boxShadow: "var(--shadow-tile)", overflow: "hidden", marginBottom: 8 }}>
      <div style={{ display: "grid", gridTemplateColumns: SVC_GRID, gap: 12, padding: "10px 16px", borderBottom: "1px solid var(--line-soft)", background: "var(--bg-base)" }}>
