...
/* global React, Icon, BrandMark, PrimaryBtn, SecondaryBtn, IconBtn */

const { useState: useSetState } = React;

// ---- Primitive controls -----------------------------------------------------

// sliding switch — used for binary on/off settings
function Switch({ checked, onChange, labelledby }) {
  return (
    <button role="switch" aria-checked={checked} aria-labelledby={labelledby}
      ...
      <span style={{
        ...
        boxShadow: "0 1px 2px rgba(23,27,29,0.35)",
...
// segmented control for short exclusive choices
function Segment({ value, options, onChange }) {
  return (
    <div style={{
      display: "inline-flex", padding: 3, gap: 3, borderRadius: 10,
      background: "var(--surface-recessed)", border: "1px solid var(--line-soft)"
    }}>
      {options.map((o) => {
    ...
    <div style={{ position: "relative", display: "inline-flex" }}>
      <select value={value} onChange={(e) => onChange(e.target.value)}
        style={{
          ...
          padding: "8px 34px 8px 12px", borderRadius: "var(--r-control)",
          border: "1px solid var(--line)", background: "var(--surface)",
          color: "var(--text-primary)", fontSize: 13, fontWeight: 600,
          fontFamily: "var(--font-sans)", cursor: "pointer", minWidth: 180
        }}>
...
function Row({ id, label, hint, children, last }) {
  return (
    <div style={{
...
function DirectoryManager() {
  const [dirs, setDirs] = useSetState([
    { path: "~/Workspace", scan: true },
    { path: "~/Code/experiments", scan: true },
    { path: "D:\\Archive\\projects", scan: false }
  ]);
  const [draft, setDraft] = useSetState("");
  ...
  return (
    <div>
      <div style={{ padding: "6px 0" }}>
        {dirs.map((d, i) => (
          <div key={d.path + i} style={{
            display: "flex", alignItems: "center", gap: 14,
            padding: "12px 22px", borderBottom: "1px solid var(--line-soft)"
          }}>
      ...
      <div style={{ display: "flex", gap: 10, padding: "14px 22px 18px", alignItems: "center" }}>
        <span style={{ position: "relative", flex: 1, display: "flex" }}>
          <input value={draft} onChange={(e) => setDraft(e.target.value)}
            ...
            placeholder="Add a workspace directory, e.g. ~/Projects"
            className="mono"
            style={{
              flex: 1, padding: "10px 13px", fontSize: 13,
...
function SettingsPage({ t, setTweak }) {
  const isDark = t.theme === "Basalt Dark";

  // project-management preferences (prototype-local)
  const [mgmt, setMgmt] = useSetState({
    editor: "VS Code",
  ...
  const setM = (k, v) => setMgmt((m) => ({ ...m, [k]: v }));
    ...
    <div style={{ maxWidth: 820, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20, paddingBottom: 12 }}>
      ...
      <SettingsCard title="Appearance" desc="How the Toshokan interface looks on this machine.">
        <Row id="set-theme" label="Theme" hint="Switch between the Lithic Light and Basalt Dark palettes.">
          <Segment
            ...
            onChange={(v) => setTweak("theme", v === "dark" ? "Basalt Dark" : "Lithic Light")} />
        </Row>
        ...
        <Row id="set-emblem" label="Show rail emblem" hint="Display the engraved shield mark at the base of the navigation rail." last>
          <Switch checked={t.substrate} onChange={(v) => setTweak("substrate", v)} labelledby="set-emblem" />
        </Row>
      </SettingsCard>
      ...
      <SettingsCard title="Project Management" desc="Defaults applied when launching, opening, and managing projects.">
        ...
        <Row id="set-terminal" label="Default terminal" hint="Used for “Open terminal” and launch commands.">
          <Select value={mgmt.terminal} options={["Windows Terminal", "PowerShell", "WSL — bash", "Command Prompt"]} onChange={(v) => setM("terminal", v)} />
        </Row>
        <Row id="set-branch" label="Branch to display" hint="Which branch the project header reports by default.">
          <input value={mgmt.defaultBranch} onChange={(e) => setM("defaultBranch", e.target.value)}
            className="mono"
            style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Version 0.4.0 · All data is stored locally</div>
        </div>
