...
// ---- Icon library ---------------------------------------------------------
// Simple geometric strokes on a 24px grid (per design guide §9).
const TSK_ICON_PATHS = {
...
function Icon({ name, size = 16, stroke = 1.75, style }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={stroke} strokeLinecap="round" strokeLinejoin="round" style={style} aria-hidden="true">
      {TSK_ICON_PATHS[name] || <circle cx="12" cy="12" r="7" />}
    </svg>
  );
}

// ---- Mineral node (state dot) --------------------------------------------
const TSK_NODE_COLORS = {
  running: "var(--running)",
  starting: "var(--signal)",
  stopped: "var(--line)",
  failed: "var(--error)",
...
function Node({ state = "neutral", size = 7, pulse = false }) {
  ...
  return (
    <span
      ...
      style={{
        ...
        borderRadius: "50%",
        flexShrink: 0,
        background: hollow ? "transparent" : TSK_NODE_COLORS[state],
        border: hollow ? `1.5px solid ${TSK_NODE_COLORS.stopped}` : "none",
...
// ---- Placeholder slot ------------------------------------------------------
// Reserved space for separately designed assets. Striped fill + mono label.
function Slot({ label, width = "100%", height = 48, radius = 8, style, hideLabel = false }) {
  return (
    <div
...
const TSK_OCTAGON = "polygon(29% 2%, 71% 2%, 98% 29%, 98% 71%, 71% 98%, 29% 98%, 2% 71%, 2% 29%)";

// Project icon artwork (256px masters, downscaled from supplied logos).
// Art already includes the octagonal glass specimen frame, so it renders plain.
const TSK_ICON_ART = {
  "Idolmancer": "toshokan/assets/icon-idolmancer.png",
  "Jagaimo": "toshokan/assets/icon-jagaimo.png",
  "Sando": "toshokan/assets/icon-sando.png",
  "Plastiglom": "toshokan/assets/icon-plastiglom.png",
  "Frog Budget": "toshokan/assets/icon-frogbudget.png",
  "Inventorois": "toshokan/assets/icon-inventorois.png",
  "Kani-miso": "toshokan/assets/icon-kanimiso.png"
};

function SpecimenFrame({ size = 64, label = "project icon", selected = false }) {
  const name = label.startsWith("icon: ") ? label.slice(6) : label;
  const art = TSK_ICON_ART[name];
  if (art) {
    const artDark = art.replace(".png", "-dark.png");
    // multiply (light) / screen (dark) lets the glass plaque adopt the surface
    // beneath it — including the signal tint of a selected card — so the icon
    // background blends seamlessly instead of sitting as a white square.
    const imgBase = {
  ...
  return (
    <div style={{ width: size, height: size, position: "relative", flexShrink: 0 }}>
      <div style={{
        ...
        clipPath: TSK_OCTAGON,
        background: selected
          ? "linear-gradient(155deg, var(--signal-soft), var(--surface-recessed))"
          : "linear-gradient(155deg, var(--surface), var(--surface-recessed))",
        border: "1px solid var(--line)",
        boxShadow: "inset 0 1px 0 rgba(255,255,255,0.5)"
      }}></div>
      <div style={{
        position: "absolute", inset: Math.max(3, size * 0.06),
        clipPath: TSK_OCTAGON,
        background: "var(--placeholder-stripe)",
...
function BrandMark({ size = 18, style }) {
  const imgBase = { width: size, height: size, objectFit: "contain", verticalAlign: "top" };
  return (
    <span style={{ width: size, height: size, display: "inline-block", flexShrink: 0, ...style }}>
      <img className="tsk-icon-light" src="toshokan/assets/app-icon.png" alt="Toshokan" width={size} height={size} style={{ ...imgBase, mixBlendMode: "multiply" }} />
      <img className="tsk-icon-dark" src="toshokan/assets/app-icon-dark.png" alt="Toshokan" width={size} height={size} style={{ ...imgBase, mixBlendMode: "screen" }} />
    </span>
  );
}
...
function TechChip({ label }) {
  // language/environment chip — text only, no logo glyph
  return (
    <span style={{
...
function PathChip({ path, copyable = false }) {
...
function StatusChip({ label, state = "neutral", pulse = false }) {
  return (
    <span style={{
      display: "inline-flex", alignItems: "center", gap: 6,
      padding: "3px 9px",
...
// ---- Buttons ----------------------------------------------------------------
function PrimaryBtn({ children, icon, onClick, style, big = false }) {
  ...
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{
        ...
        background: "var(--basalt)", color: "var(--basalt-text)",
...
function SecondaryBtn({ children, icon, onClick, style }) {
  const [hover, setHover] = useState(false);
  return (
    <button onClick={onClick} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)}
      style={{
...
function IconBtn({ icon, title, onClick, size = 30, disabled = false, active = false }) {
  const [hover, setHover] = useState(false);
  return (
    <button title={title} aria-label={title} onClick={onClick} disabled={disabled}
      ...
      style={{
        ...
        display: "flex", alignItems: "center", justifyContent: "center",
        borderRadius: 7,
        border: "1px solid " + (active ? "var(--signal)" : "var(--line-soft)"),
...
function Toast({ message }) {
  ...
  return (
    <div style={{
      ...
      boxShadow: "var(--shadow-raised)", zIndex: 300,
...
Object.assign(window, { Icon, Node, Slot, SpecimenFrame, BrandMark, TechChip, PathChip, StatusChip, PrimaryBtn, SecondaryBtn, IconBtn, Toast, TSK_OCTAGON });
